...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        pare = rest.map(h)
        return scheme_apply(f, pare ,env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Parameters don\'t match')
        while formals != nil:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Wrond number of parameters are passed in')        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        v = scheme_eval(expressions.first, env)
        if v is False:
            return v
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        check_form(bind, 2, 2)
        check_formals(formals)
        if scheme_symbolp(bind.first):
            val1 = scheme_eval(bind.second.first, env)
            formals, vals = Pair(bind.first, formals), Pair(val1, vals)
            bindings = bindings.second
        else:
            raise SchemeError('The thing that you want the value to bind to is not a symbol')
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            def h(expr):
                return scheme_eval(expr, env)
            pare = rest.map(h)
            val = scheme_apply(f, pare ,env)
            result = complete_eval(val)
            # END Extra Credit
    return result
